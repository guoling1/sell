...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      _initScroll(){
        this.$refs.menuScroll = new BScroll(this.$refs.menuWrapper, {
            click:true
        });
        this.$refs.foodScroll = new BScroll(this.$refs.foodWrapper, {
          probeType: 3
        })
        this.$refs.foodScroll.on('scroll', (pos) => {
          this.scrollY = Math.abs(Math.round(pos.y));
        })
      },
      _calculateHeight(){
        let footList = this.$refs.foodWrapper.getElementsByClassName('food-list-hook');
        let height = 0;
